    ...
    for (let d = 0; d < DIAS.length; d++) {
      ...
      const contenido = aqui.map((it) => {
        ...
        if (it.tipo === "curso") {
          ...
          return `<div style="background:${bg};color:${tx};border-left:4px solid ${bc};border-radius:5px;padding:4px 6px;margin-bottom:2px;box-shadow:0 1px 2px rgba(0,0,0,0.12);${fs};min-height:28px;overflow:hidden"><div style="font-weight:700;font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escaparHTML(it.codigo ?? "")}${grupo}</div><div style="font-size:10px;margin-top:1px;line-height:1.1;overflow:hidden;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;white-space:normal;word-wrap:break-word">${escaparHTML(it.nombre)}</div><div style="font-size:9px;opacity:.75;margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escaparHTML(it.aula)}</div></div>`;
        }
        const borde = it.flexible ? "dashed" : "solid";
        return `<div style="background:${it.color.bg};color:${it.color.text};border:1px ${borde} ${it.color.border};border-radius:12px;padding:3px 6px;margin-bottom:2px;width:100%;overflow:hidden"><div style="font-size:10px;line-height:1.1;overflow:hidden;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;white-space:normal;word-wrap:break-word">${it.icono ?? ""} ${escaparHTML(it.nombre)}</div><div style="font-size:9px;opacity:.75;margin-top:1px">${it.inicio}–${it.fin}</div></div>`;
      }).join("");
